...
let decode_chain ~str ~msg = 
  ...
  | Ok _ ->
    assert_failure "Impossible to decode an unsupported codec.";
  | Error s ->
    assert_equal ~printer:Fun.id msg s)

let tests = [
"test codec chain" >:: (fun _ ->
  let decoded_repr : (float, Bigarray.float32_elt) array_repr =
    {shape = [|10; 15; 10|]
  ...
  let chain =
    ...
    ;b2b = [Crc32c; Gzip L9]}
  in
  assert_bool "" @@ Result.is_error @@ Chain.create decoded_repr chain; 
  ...
  assert_bool
    "enc should be successfully encoded" @@ Result.is_ok enc;
  let encoded = Result.get_ok enc in
  (match Chain.decode c decoded_repr encoded with
  | Ok v ->
    assert_bool "" @@ Ndarray.equal arr v;
  | Error _ ->
    assert_failure "Successfully encoded array should decode without fail");
...
"test transpose codec" >:: (fun _ ->
  ...
  let decoded_repr : (Complex.t, Bigarray.complex32_elt) array_repr =
    {shape = [|2; 2; 2|]
